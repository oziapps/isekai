...
4. **"Ücretsiz ve sınırsız" bir AI 3D SaaS yok.** Meshy'nin ücretsiz planı 100 kredi/ay veriyor ve çıktılar **CC BY 4.0** lisanslı, atıf zorunlu [B][8][10]. **(düzeltildi 24.09.2026)** Meshy Yardım Merkezi'ne göre ücretsiz planda ayda yalnızca **10 indirme** var ve bunlar yalnızca **Meshy 6 Lite** modelleri için geçerli. **Meshy 6 ve Meshy 7 ile üretilen modeller ücretsiz planda indirilemiyor.** CC BY 4.0 atıfla ticari kullanıma izin veriyor, ama model herkese açık ve size özel değil. Tripo'nun ücretsiz Basic planı yaklaşık 300 kredi/ay; çıktılar herkese açık ve CC BY 4.0 [B][11]. **(düzeltildi 24.09.2026)** Tripo'nun resmî fiyat sayfası ücretsiz plan modellerini "herkese açık ve **ticari olmayan**" olarak etiketliyor. İkincil kaynaklarda aylık kredi 200 ile 300 arasında değişiyor. Rodin'in ücretsiz katmanında dışa aktarım hakkı yok, bu hak Creator planında (30 $/ay) başlıyor [B][10]. **(düzeltildi 24.09.2026)** Rodin'in ücretsiz planında yalnızca eski (legacy) modelden dışa aktarım var, bunun dışında indirme başına kredi ödeniyor (≈1,50 $/kredi). Sınırsız dışa aktarım Creator planında başlıyor. Ticari oyun için ya ücretli bir ay ya da açık ağırlıklı model gerekiyor.
5. **Gerçekten ücretsiz ve sınırsız yol, açık ağırlıkları kendi GPU'nda çalıştırmak.** TRELLIS.2 **MIT** lisanslı (4B parametre, PBR, ≥24 GB VRAM) [A][16]. **(düzeltildi 24.09.2026) Uyarı:** TRELLIS.2'nin kodu ve ağırlıkları MIT, ama GLB/doku dışa aktarımı (`o_voxel.postprocess.to_glb`) **nvdiffrast** kütüphanesini çağırıyor. Kurulum betiği ayrıca nvdiffrec'i de kuruyor. İkisi de "Nvidia Source Code License" altında ve **yalnızca araştırma ve değerlendirme (ticari olmayan) kullanıma** izin veriyor. Repoda bu konuda açık bir soru da var (issue #22). Ticari asset üretiminden önce hukuki inceleme ya da rasterizer'ı değiştirmek gerekiyor. TripoSR (MIT, ~6 GB) ve TripoSG (MIT, ≥8 GB) daha hafif seçenekler [A][17]. **(düzeltildi 24.09.2026)** TripoSG'nin hazır hattı arka plan temizleme için BRIA **RMBG-1.4** modelini indiriyor. Bu model **ticari olmayan** lisanslı. Ticari kullanımda maske önceden hazırlanmalı ya da `rembg` gibi MIT/Apache bir araç kullanılmalı. TripoSR'nin bağımlılıklarında böyle bir kısıt görülmedi. Bulut konteynerimizde GPU yok ve huggingface.co engelli, dolayısıyla bu yol ancak kullanıcının kendi PC'sinde (ya da kiralık GPU'da) mümkün.
...
|---|---|---|---|---|---|
| **Meshy** (Meshy-6) | 100 kredi/ay; çıktılar **CC BY 4.0** (atıf zorunlu); API yalnızca ücretli (Pro+). **(düzeltildi 24.09.2026)** Ücretsiz planda ayda 10 indirme var, yalnızca Meshy 6 Lite modelleri için. Meshy 6/7 modelleri indirilemiyor. Pro: 20 $/ay, 1.000 kredi | Ücretli planda tam özel sahiplik (Community'de yayınlamazsak) | Rig 5 kr, animasyon 3 kr; kütüphane `action_id`. Rig için dokulu insansı model, A/T poz ve en fazla 300k yüz şartı. Text-to-motion (FBX/BVH, 2–10 sn) | "Smart topology" 100–15.000 üçgen, quad remesh | [B][8][9][10] |
| Meshy API maliyetleri | — | — | — | Image-to-3D dokulu 30 kr (Meshy-6), remesh 5 kr | [B][8] (2026-07-10) |
| **Tripo** (v3.x) | Yeni hesaba 300 kredi (~2 hafta geçerli). Basic ≈300 kr/ay, 1 eşzamanlı iş; ücretsiz çıktılar herkese açık, CC BY 4.0. **(düzeltildi 24.09.2026)** Resmî fiyat sayfası ücretsiz modelleri "public, **non-commercial**" olarak etiketliyor. Kredi miktarı kaynağa göre 200–300/ay | Ücretli planlarda özel model ve ticari hak | `check_riggable` ücretsiz, rig 25 kr, retarget 10 kr/hareket. Hazır hareketler: idle, walk, run, jump, climb, slash, shoot, hurt… | Quad +5 kr, smart low-poly +10 kr | [B][11] |
| Tripo API fiyatı | $1 = 100 kredi; image→3D 20–30 kr (v3) | — | — | — | [B][11] |
| Tripo abonelik | Pro ≈15,90 $/ay (~3.000 kr), Advanced ≈39,90 $/ay (~8.000 kr). **(düzeltildi 24.09.2026)** Bunlar yıllık faturalandırmadaki aylık fiyatlar. **Aylık** faturalandırmada Professional 19,90 $, Advanced 49,90 $ | — | — | — | [B][11], değişken |
| **Rodin / Hyper3D** (Gen-2) | Ücretsiz: "10 private assets", dışa aktarım hakkı yok. **(düzeltildi 24.09.2026)** Ücretsiz planda eski (legacy) modelden dışa aktarım ve indirme başına ödeme (≈1,50 $/kredi) var | Creator 30 $/ay'dan itibaren dışa aktarım; API Business 120 $/ay | Yerleşik rig yok (T/A poz zorlama var) | Yüksek detay ve PBR | [B][10], [C][21] |
...
|---|---|---|---|---|
| **TRELLIS.2-4B** (Microsoft) | **MIT** (HF ağırlık kartı da MIT). **(düzeltildi 24.09.2026)** GLB/doku dışa aktarımı nvdiffrast'a bağlı (NVIDIA Source Code License, yalnızca ticari olmayan kullanım). Ticari kullanım gri alanda | ≥24 GB NVIDIA (A100/H100'de doğrulanmış) | Image→3D, tam PBR, 512³ ≈3 sn / 1024³ ≈17 sn / 1536³ ≈60 sn (H100); doku üretimi ve eğitim kodu dahil | 2026-06-05 [A][16] |
...
| **TripoSG** | MIT. **(düzeltildi 24.09.2026)** Hattın indirdiği arka plan temizleyici RMBG-1.4 ticari olmayan lisanslı, değiştirilmeli | ≥8 GB | Yüksek kaliteli şekil (doku yok) | 2025-04-18 [A][17] |
...
| **KayKit Character Animations** | 161 klip (movement, melee, ranged…). (Not, 24.09.2026: itch.io'daki "Legacy" sürüm 133 klip; sayı sürüme göre değişiyor) | **CC0** (itch.io sayfasında teyit edildi) | [B][28] |
...
| **Rokoko Vision (Create)** | Videodan harekete ve metinden harekete | Ücretsiz Starter: **30 sn/ay** Vision işleme, FBX dışa aktarım; "all generated data… can be used commercially". (24.09.2026'da teyit edildi: Vision 3.0 artık Rokoko Create içinde. Plus planı aylık 28 $, yıllıkta 20 $/ay, 600 sn/ay) | [B][30] |
...
| **Cascadeur** | Fizik destekli keyframe, AutoPosing ve ara kare üretimi | Ücretsiz ve "Indie" katmanları var; eşik ve fiyat **doğrulanamadı**. 2026.2 (Ağustos 2026) additive katman ve Python API getirmiş [C]. **(düzeltildi 24.09.2026)** Ücretsiz plan **ticari kullanıma kapalı** ve yalnızca kendi `.casc` biçimine dışa aktarıyor (FBX/glTF yok). **Indie** planı yıllık brüt gelir veya fonlama **100.000 $ altı** için; FBX/DAE/USD/glTF dışa aktarımı var. Fiyatı ikincil kaynaklara göre ≈12 €/ay. Pro ≈49 €/ay | [O] cascadeur.com/plans (arama özeti) |

### 4.2 Açık metinden-harekete (text-to-motion) modeller

| Model | Lisans | VRAM | Çıktı/iskelet | Uygunluk |
|---|---|---|---|---|
| **Kimodo** (NVIDIA, 2026-03) | Kod Apache-2.0; SOMA ve G1 ağırlıkları **NVIDIA Open Model License**; SMPL-X varyantı yalnızca Ar-Ge lisansıyla | ≈17 GB; `TEXT_ENCODER_DEVICE=cpu` ile <3 GB | SOMA 77 eklemli iskelet. Metin + keyframe + yol kısıtları, zaman çizelgesi editörü | **En umut verici seçenek.** "Commercially-friendly" 700 saatlik mocap ile eğitilmiş. Model lisansının ticari şartları doğrulanmalı [A][34]. (24.09.2026: NVIDIA Open Model License modellerin ticari kullanılabildiğini ve NVIDIA'nın çıktılar üzerinde sahiplik iddia etmediğini söylüyor. Kullanıcıya üçüncü taraf taleplerine karşı tazmin yükümlülüğü getiriyor. Metin kodlayıcı LLM2Vec'in alt model lisansı ayrıca kontrol edilmeli) |
...
| **Önerilen üretim ayları:** + Meshy Pro veya Tripo Pro + DeepMotion/Rokoko alt ücretli plan + Cascadeur Indie | ≈16 $ (Tripo Pro [B]) + 9–39 $ (DeepMotion [B]) + Cascadeur (doğrulanamadı). Toplam yaklaşık **30–80 $/ay**. **(düzeltildi 24.09.2026)** Aylık faturada Tripo Pro ≈19,90 $, Meshy Pro 20 $, Cascadeur Indie ≈12 €/ay [O]. Toplam yaklaşık **40–90 $/ay** | Prop ve canavarlarda hızlı üretim, imza hareketlerde mocap, ticari haklar net |
...
3. **Hunyuan3D ve HY-Motion çıktıları ticari yapıya hiç girmemeli.** Oyun Steam'de AB, İngiltere ve Kore'ye de satılacak, lisans ise çıktının bu bölgelerde "display" edilmesini yasaklıyor. Güvenli açık alternatifler: TRELLIS.2 (MIT), TripoSG/TripoSR (MIT) ve Kimodo (Apache-2.0 kod + NVIDIA Open Model). **(düzeltildi 24.09.2026)** TRELLIS.2 "koşullu" sayılmalı: GLB/doku dışa aktarımı ticari olmayan lisanslı nvdiffrast'a bağlı, hukuki inceleme ya da rasterizer değişikliği gerekiyor. TripoSG'de RMBG-1.4 (ticari olmayan) arka plan temizleyicisi değiştirilmeli. Bağımlılık açısından en temiz görünen seçenek TripoSR.
4. **AI 3D SaaS'ı "üretim sprint'i" modeliyle kullanalım.** Asset listesi önceden hazırlanır, ardından bir ya da iki ay Tripo Pro (≈16 $) veya Meshy Pro alınıp toplu üretim yapılır. Ücretsiz plan çıktıları (CC BY, herkese açık) yalnızca prototipte kullanılmalı. **(düzeltildi 24.09.2026)** Aylık faturada Tripo Pro ≈19,90 $, Meshy Pro 20 $. Tripo ücretsiz modelleri resmî olarak "ticari olmayan". Meshy'nin ücretsiz planı Meshy 6/7 modellerini hiç indirmiyor. Bu yüzden ücretsiz planlar prototip için bile çok sınırlı.
...
- **Doğrulanamayanlar:** Cascadeur fiyatı ve lisans eşikleri (24.09.2026: eşik ve koşullar doğrulandı, bkz. §4.1; fiyat ikincil kaynaktan); Move.ai, QuickMagic, Plask, CSM, Kaedim ve Sparc3D. Mixamo'nun uzun vadede ayakta kalıp kalmayacağı da belirsiz (güncellenmiyor).
...
Bağımsız kontrol: lisans dosyaları raw.githubusercontent.com'dan doğrudan okundu. Yıldızlar ve son push tarihleri GitHub arama API'siyle (`pushed:` filtresi) kontrol edildi. SaaS fiyatları için resmî siteler engelli olduğundan sunucu tarafı web aramasının resmî sayfa özetleri kullanıldı.

| İddia | Sonuç | Düzeltme/Not | Kaynak |
|---|---|---|---|
| VRoid Studio yönergeleri: VRoid mesh/doku/preset'ler oyunlarda ticari kullanılabilir; içerik CC0 değil; VRoid mesh'lerinden model üreten uygulama yapmak ayrı lisans istiyor | Doğrulandı | Ek bilgi: Uygulama yalnızca sizin kendi kullanımınız içinse ayrı lisans gerekmiyor. Oyuncuya açık karakter editörü için yasak geçerli | https://vroid.com/en/studio/guidelines (arama özeti), https://vroid.pixiv.help/hc/en-us/articles/4405813333657 |
| VRM 1.0 meta varsayılanları: `allowExcessivelyViolentUsage=false`, `commercialUsage=personalNonProfit`, `modification=prohibited`, `allowRedistribution=false`, `creditNotation=required` | Doğrulandı | — | https://github.com/vrm-c/vrm-specification/blob/master/specification/VRMC_vrm-1.0/meta.md |
| Meshy ücretsiz planı: 100 kredi/ay, çıktılar CC BY 4.0 | Düzeltildi | Kredi ve lisans doğru. Ancak ücretsiz planda yalnızca Meshy 6 Lite modelleri için ayda 10 indirme var, Meshy 6/7 modelleri indirilemiyor. CC BY 4.0 atıfla ticari kullanıma izin veriyor. Pro: 20 $/ay, 1.000 kredi | https://help.meshy.ai/en/articles/15696428-what-is-included-on-the-free-plan , https://www.meshy.ai/pricing |
| Tripo ücretsiz çıktılar herkese açık ve CC BY 4.0; Pro ≈15,90 $/ay | Düzeltildi | Resmî fiyat sayfası ücretsiz modelleri "public, non-commercial" diye etiketliyor. Kredi 200–300/ay (kaynaklar çelişiyor). Aylık faturada Professional 19,90 $ (3.000 kr), Advanced 49,90 $. 15,90/39,90 yıllık faturalandırma fiyatları | https://www.tripo3d.ai/pricing |
| Rodin ücretsiz katmanında dışa aktarım yok; Creator 30 $, API Business 120 $ | Düzeltildi (küçük) | Fiyatlar doğru. Ücretsiz planda eski modelden dışa aktarım ve indirme başına ödeme (≈1,50 $/kredi) var | https://hyper3d.ai/pricing |
| TRELLIS.2 MIT, ≥24 GB, 512³ ≈3 sn / 1024³ ≈17 sn (H100) | Düzeltildi (lisans uyarısı) | Kod ve HF ağırlıkları MIT. Ancak GLB dışa aktarımı `nvdiffrast` import ediyor, kurulum betiği nvdiffrec'i de kuruyor. Bu NVIDIA lisansları yalnızca araştırma ve değerlendirme amaçlı (ticari olmayan) kullanıma izin veriyor. Repoda issue #22 bu konuyu soruyor. "Güvenli" değil, "koşullu" | https://github.com/microsoft/TRELLIS.2 (README, `o-voxel/o_voxel/postprocess.py`, `setup.sh`), https://github.com/NVlabs/nvdiffrast/blob/main/LICENSE.txt , https://github.com/microsoft/TRELLIS.2/issues/22 |
| TripoSG MIT, ≥8 GB | Düzeltildi (bağımlılık uyarısı) | Model MIT. Ancak hazır hat arka plan temizleme için briaai/RMBG-1.4'ü indiriyor; bu model ticari olmayan lisanslı. TripoSR (MIT) `rembg` kullanıyor | https://github.com/VAST-AI-Research/TripoSG (README, requirements), https://huggingface.co/briaai/RMBG-1.4 |
| Hunyuan3D-2.1 lisansı AB/UK/Güney Kore'yi hariç tutuyor ve çıktının bölge dışında kullanılmasını/gösterilmesini yasaklıyor; >1M MAU'da ayrı lisans | Doğrulandı | Madde "must not use, reproduce, modify, distribute, or display … Output … outside the Territory" | https://raw.githubusercontent.com/Tencent-Hunyuan/Hunyuan3D-2.1/main/LICENSE |
| HY-Motion 1.0 (30.12.2025) aynı bölge istisnası; 26 GB (Lite 24 GB) | Doğrulandı | License.txt aynı bölge ve çıktı maddelerini içeriyor | https://github.com/Tencent-Hunyuan/HY-Motion-1.0 (README, License.txt) |
| Mixamo: Adobe ID ile ücretsiz, oyunlarda telifsiz ticari kullanım | Doğrulandı | Ek bilgi: Enterprise/Federated ID'lerde ve Çin ülke kodlu hesaplarda kullanılamıyor | https://helpx.adobe.com/creative-cloud/faq/mixamo-faq.html |
| Quaternius UAL 2: 130+ klip, yakın dövüş kombolar, CC0 | Doğrulandı | 3 ve 4 vuruşluk kombolar, parçalı vuruşlar dahil | https://quaternius.itch.io/universal-animation-library-2 , https://quaternius.com/packs/universalanimationlibrary2.html |
| KayKit Character Animations CC0, 161 klip | Doğrulandı (sayı notuyla) | CC0 ve atıf gerekmiyor. "Legacy" pakette 133 klip var, sayı sürüme göre değişiyor | https://kaylousberg.itch.io/kaykit-character-animations |
| Cascadeur 2026 ücretsiz ve Indie şartları doğrulanamadı | Düzeltildi (artık doğrulandı) | Ücretsiz plan ticari olmayan kullanım için ve yalnızca `.casc` dışa aktarıyor. Indie: yıllık gelir veya fonlama <100.000 $, tüm dışa aktarım biçimleri açık, ≈12 €/ay (ikincil). Pro ≈49 €/ay | https://cascadeur.com/plans , https://cascadeur.com/blog/general/cascadeurs-new-licensing-structure-comprehensive-faq |
| Rokoko Vision ücretsiz Starter: 30 sn/ay, FBX, ticari kullanım serbest | Doğrulandı | Vision 3.0 artık Rokoko Create içinde. Plus: aylık 28 $ (yıllıkta 20 $/ay), 600 sn/ay | https://www.rokoko.com/pricing |
| DeepMotion Freemium ticari değil, 60 sn/ay; Starter 9 $ | Doğrulandı | Diğer kademelerin fiyatları bu turda tekrar kontrol edilmedi | https://www.deepmotion.com/pricing-animate3d |
| NVIDIA Kimodo var; kod Apache-2.0, SOMA/G1 ağırlıkları NVIDIA Open Model, SMPL-X R&D; ~17 GB / CPU metin kodlayıcıyla <3 GB | Doğrulandı | NVIDIA Open Model License ticari kullanıma izin veriyor ve çıktılar üzerinde sahiplik iddia etmiyor (tazmin maddesi var). SOMA-X iskeleti Apache-2.0. Metin kodlayıcı LLM2Vec'in alt model lisansı kontrol edilmedi | https://github.com/nv-tlabs/kimodo , https://www.nvidia.com/en-us/agreements/enterprise-software/nvidia-open-model-license/ |
| GVHMR yalnızca eğitim, araştırma ve kâr amacı gütmeyen kullanım için | Doğrulandı | "prohibited for commercial use" | https://github.com/zju3dv/GVHMR/blob/main/LICENSE |
| Audio2Face-3D: SDK ve UE5 eklentisi MIT, modeller NVIDIA Open Model, Audio2Emotion yalnızca A2F ile | Doğrulandı | Training Framework Apache, NIM ayrı NVIDIA lisansı | https://github.com/NVIDIA/Audio2Face-3D |
| ahujasid/mcp-for-blender: 29.284★, MIT, son push 2026-09-24 | Doğrulandı | PyPI paketi `mcp-for-blender`; Rodin ve Hunyuan3D entegrasyonu README'de | https://github.com/ahujasid/mcp-for-blender |
| CoplayDev/unity-mcp: 14.461★, MIT, v10.0.0 (2026-06-30), Unity 2021.3–6.x | Doğrulandı | Arama anında 14.460★. Son push ≥2026-09-20 | https://github.com/CoplayDev/unity-mcp |
| Coding-Solo/godot-mcp: 5.811★, MIT, son commit 2026-04-16 | Doğrulandı | Son push 2026-04-16, sonrasında push yok (5 aydır sessiz) | https://github.com/Coding-Solo/godot-mcp |
| chongdashu/unreal-mcp: son commit 2025-04-22 | Doğrulandı | 2025-04-23 sonrasında push yok | https://github.com/chongdashu/unreal-mcp |
| Effekseer MIT; örnek efektler CC0 | Doğrulandı (nüansla) | Araç MIT. Resmî sayfaya göre örneklerin "neredeyse tamamı" CC0, yani her örnek tek tek kontrol edilmeli | https://github.com/effekseer/Effekseer/blob/master/LICENSE , https://effekseer.github.io/en/contribute.html |
| PyPI `bpy` 5.2.2 (Python 3.13) | Doğrulandı | `requires_python ==3.13.*` | https://pypi.org/pypi/bpy/json |
